...
State::State(SamplingState* astate)
{
...
State::State(int* values, unsigned int n_species)
{
	this->sn = 0;
	this->n_species = n_species;
	state = new STATETYPE[this->n_species];
...
int State::PrintState(ofstream &f)
{
	for (unsigned int j=0; j<this->n_species; j++)
        {
		f << this->getStateValue(j) << " ";
	}
	f << endl;
...
unsigned int State::getNumSpecies()
{
